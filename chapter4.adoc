...
5+h|0x0008
h|Field                             |Bits   |R/W    |Default   |Description
|{set:cellbgcolor:#FFFFFF} `enable` |0:0    |W1SS   |IMP       |Indicate if the IOPMP checks transactions by default. If it is implemented, it should be initial to 0 and sticky to 1. If it is not implemented, it should be wired to 1.
...
|`no_err_rec`                       |23:23  |R      |IMP       |Indicate if error record (`ERR_INFO`, `ERR_REQADDR`, `ERR_REQADDRH`, `ERR_REQID` and `ERR_USER(0:7)`) is not implemented.
...
h|0x0014
|{set:cellbgcolor:#FFFFFF} See xref:APPLICATION_NOTE[xrefstyle=full].
|===
...
5+h|0x0040
h|Field                         |Bits       |R/W    |Default    |Description
|{set:cellbgcolor:#FFFFFF} `l`  |0:0        |W1SS   |IMP        | Lock bit to `MDLCK` and `MDLCKH` register.
...
5+h|0x0044
h|Field                         |Bits       |R/W    |Default    |Description
|{set:cellbgcolor:#FFFFFF} `mdh`|31:0       |WARL   |IMP        | `mdh[_m_]` is sticky to 1 and indicates if `SRCMD_ENH(_s_).mdh[_m_]` is locked for all RRID _s_.
|===
...
5+h|0x0048
h|Field                         |Bits       |R/W    |Default    |Description
|{set:cellbgcolor:#FFFFFF} `l`  |0:0        |W1SS   |IMP        | Lock bit to `MDCFGLCK` register.
...
5+h|0x004C
h|Field                         |Bits       |R/W    |Default    |Description
|{set:cellbgcolor:#FFFFFF} `l`  |0:0        |W1SS   |IMP        | Lock bit to `ENTRYLCK` register.
...
5+h|0x0060
h|Field                           |Bits   |R/W    |Default    |Description
|{set:cellbgcolor:#FFFFFF} `l`    |0:0    |W1SS   |IMP        |Lock bit to `ERR_CFG` register.
|{set:cellbgcolor:#FFFFFF} `ie`   |1:1    |WARL   |IMP        |Enable the interrupt of the IOPMP rule violation.
|{set:cellbgcolor:#FFFFFF} `rs`   |2:2    |WARL   |IMP       a| 
...
`v`                             .2+|0:0    |R     |0       | Indicate if the illegal capture recorder (`ERR_REQID`, `ERR_REQADDR`, `ERR_REQADDRH`, `ERR_INFO.ttype`, and `ERR_INFO.etype`) has a valid content and will keep the content until the bit is cleared. An interrupt will be triggered if a violation is detected and related interrupt enable/suppression configure bits are not disabled, the interrupt will keep asserted until the error valid is cleared.
                                           |W1C   |N/A     | Write 1 clears the bit, the illegal recorder reactivates and the interrupt (if enabled). Write 0 causes no effect on the bit.
...
|{set:cellbgcolor:#FFFFFF} `eid`  |31:16      |R      |DC         |Indicates the index pointing to the entry that catches the violation. If no entry is hit, for example, `ERR_INFO.etype`= 0x05 or 0x06, the value of this field is invalid. If the field is not implemented, it should be wired to 0xffff.
|===
...
5+h|0x1000 + (_s_) × 32
h|Field                         |Bits       |R/W    |Default    |Description
|{set:cellbgcolor:#FFFFFF} `l`  |0:0        |W1SS   |IMP        | A sticky lock bit. When set, locks `SRCMD_EN(_s_)` and `SRCMD_ENH(_s_)`.
