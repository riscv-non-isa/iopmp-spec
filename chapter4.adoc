...
5+h|0x0008
h|Field                             |Bits   |R/W    |Default   |Description
|{set:cellbgcolor:#FFFFFF} `enable` |0:0    |W1SS   |IMP       |Indicate if the IOPMP checks transactions by default. If it is programmable, it must be initialized to 0 and sticky to 1. If it is not programmable, it must be wired to 1.
...
|{set:cellbgcolor:#FFFFFF} `eid`  |31:16      |R      |DC         |Indicates the index pointing to the entry that catches the violation. If no entry is hit, for example, `ERR_INFO.etype`= 0x05 or 0x06, the value of this field is invalid. If the field is not implemented, it must be wired to 0xffff.
|===
