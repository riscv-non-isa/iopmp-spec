...
|{set:cellbgcolor:#FFFFFF} MDLCK/ (optional) MDLCKH | Lock registers for SRCMD Table.
...
| ERR_REQADDR/ (optional) ERR_REQADDRH | Indicates request address regarding the first captured violation.
...
|(Optional) ENTRY_ADDRH(_i_) | Bit 65:34 of the address (region) for entry _i_. 
...
5+h|0x0044
h|Field                         |Bits       |R/W    |Default    |Description
|{set:cellbgcolor:#FFFFFF}mdh   |31:0       |WARL   |0          | *mdh[_m_]* is sticky to 1 and indicates if *SRCMD_ENH(_s_).mdh[_m_]* is locked for all RRID _s_. Available when *HWCFG0.md_num* > 31.
|===
