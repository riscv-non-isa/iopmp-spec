...
The IOPMP specification provides an extension to the baseline SRCMD Table implementation to support advanced use cases requiring flexible permission management.
// Temporarily commented alternative SRCMD Table (Format 2) due to moving to application note.
// These extensions enable different RRIDs to have varying access permissions within shared memory domains or alternative table indexing schemes.

==== Secondary Permission Setting (SPS)

IOPMP/SPS (Secondary Permission Setting) is an extension that enables different transaction requestors to share memory domains while allowing each requestor to have different read, write, and instruction fetch permissions to a single memory domain. The field *HWCFG2.sps_en*=1 indicates that the IOPMP/SPS extension is implemented.
...
.An example block diagram of an IOPMP with SPS extension. It illustrates the checking flow of an IOPMP with SPS extension. The IOPMP first looks up the SRCMD Table according to the RRID carried by the incoming transaction to retrieve associated MD indexes and the corresponding permissions related to these MDs. By the MD indexes, the IOPMP looks up the MDCFG Table to get the belonging entry indexes. The final step checks the access right according to the above entry indexes and corresponding permissions.
ifdef::backend-pdf[]
