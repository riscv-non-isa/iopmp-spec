...
`msi_werr`                     .2+|3:3    |R      |0          |It's asserted when the write access to trigger an IOPMP originated MSI has failed. When it's not available, it should be ZERO.
                                          |W1C    |N/A        |Write 1 clears the bit. Write 0 causes no effect on the bit.
|===
