...
This section introduces an extension and describes how to use the extension to update IOPMP's settings in runtime.
...
*HWCFG2.stall_en* indicates if the IOPMP implements the extension, which are *MDSTALL*, *MDSTALLH*, *RRIDSCP*, *ERR_CFG.stall_violation_en*, and addtional error type = 0x07 "error due to a stalled transaction".
