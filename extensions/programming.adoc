...
This section introduces programming protection extension and describes how to use the extension to update IOPMP's settings in runtime.
...
*HWCFG0.stall_en* indicates if the IOPMP implements Programming Protection extension, which are *MDSTALL*, *MDSTALLH*, *RRIDSCP*, *ERR_CFG.stall_violation_en*, and addtional error type = 0x07 "error due to a stalled transaction".

All registers and fields described in <<#Program_IOPMPs>> (*MDSTALL*, *MDSTALLH*, *RRIDSCP*, and *ERR_CFG.stall_violation_en*) are optional. Moreover, these features could be partially implemented.
