...
|{set:cellbgcolor:#FFFFFF} `no_x` |12:12 |R |IMP |For `HWCFG2.chk_x`=1, when `no_x`=1, the IOPMP denies all instruction fetch transactions; otherwise, it depends on the `x`-bit in `ENTRY_CFG(_i_)`. For `chk_x`=0, `no_x` has no effect.
|===
