...
In this scenario, instruction permission checking is not required.
...
or for some system bus that don't provide any instruction fetch hint,
...
[NOTE]
====
A system with a single RISC-V hart that includes PMP and only data-only bus initiators can enforce execution permission checking entirely within the hart using its PMP.
