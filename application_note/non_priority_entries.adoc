...
When the extension is enabled, IOPMP entries are partially prioritized. Entries with indices less than the value defined in a field *prio_entry* are prioritized. Entries with indices greater than or equal to *prio_entry* are treated equally and assigned the lowest priority. These are referred to as non-prioritized entries. The value of *prio_entry* is implementation-dependent. Additionally, a field *prient_prog* indicates whether *prio_entry* is programmable.
