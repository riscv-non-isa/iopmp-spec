...
The global instruction fetch behavior is controlled by the *chk_x* bit on the *HWCFG0* register *no_x* bit in the *HWCFG2* register:
...
|{set:cellbgcolor:#FFFFFF}no_x|11:11  |R     | IMP| For *HWCFG0.chk_x*=1, when *no_x*=1, the IOPMP denies all instruction fetch transactions; otherwise, it depends on the *x*-bit in *ENTRY_CFG(_i_)*. For *chk_x*=0, *no_x* has no effect
|===