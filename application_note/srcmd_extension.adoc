...
The IOPMP specification provides extensions to the baseline SRCMD Table implementation to support advanced use cases requiring flexible permission management. These extensions enable different transaction requestors to have varying access permissions within shared memory domains or alternative table indexing schemes.
