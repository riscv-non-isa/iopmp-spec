...
The two fields `MDLCK.md` and `MDLCKH.mdh` have 63 bits together. Every bit is used to lock the MD association bits in the corresponding SRCMD Table entry. For MD 0 &#x2264; _m_ &#x2264; 30, `MDLCK.md[_m_]` locks `SRCMD(_s_).md[_m_]` for all existing RRID _s_. For MD 31 &#x2264; _m_ &#x2264; 62, one should use `MDLCKH.mdh` to lock corresponding bits.
