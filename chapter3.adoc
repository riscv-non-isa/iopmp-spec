...
This chapter details the protection mechanisms provided by the IOPMP specification. These mechanisms allow implementations to prohibit modification of the configuration, either wholly or partially, based on the security requirements of the application scenario. Specifically, the IOPMP defines distinct write-protection controls for the SRCMD Table, the MDCFG Table, and the Entry Array.
...
The two fields `MDLCK.md` and `MDLCKH.mdh` have 63 bits together. Every bit is used to lock the association bits with a memory domain in the SRCMD Table. For MD 0 &#x2264; _m_ &#x2264; 30, `MDLCK.md[_m_]` locks `SRCMD(_s_).md[_m_]` for all existing RRID _s_. For MD 31 &#x2264; _m_ &#x2264; 62, one should use `MDLCKH.mdh` to lock corresponding bits.
