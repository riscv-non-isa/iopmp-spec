...
extern reg_intf_dw read_register(uint16_t offset, uint8_t num_bytes);
extern void write_register(uint16_t offset, reg_intf_dw data, uint8_t num_bytes);
...
typedef union {
    ...
    struct {
        uint32_t is_stalled : 1;                // After the last writing of MDSTALL (included) plus any following writing RRIDSCP, 1
